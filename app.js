...
const PRESETS = [
  { group: "WEB", id: "web-640", label: "WEB用・軽量", width: 640, height: null },
...
function dimensions(source = null) {
...
function syncFormatControls() {
  ...
  $("format").disabled = isWeb640();
  $("transparent").disabled = jpeg;
  $("transparentRow").classList.toggle("disabled", jpeg);
  $("qualityRow").hidden = !jpeg;
}

function applyPresetDefaults(previousPresetId) {
  if (isWeb640()) {
    state.fit = "contain";
    $("format").value = "jpeg";
    $("quality").value = "85";
    $("qualityValue").textContent = "85%";
  } else if (previousPresetId === "web-640") {
    $("format").value = "png";
    $("quality").value = "90";
      ...
      <div class="preset-grid">
        ${PRESETS.filter((item) => item.group === group).map((item) => `
          <button type="button" class="preset-card ${state.presetId === item.id ? "selected" : ""}" data-preset="${item.id}" aria-pressed="${state.presetId === item.id}">
            ...
            <small>${item.id === "custom" ? "幅・高さを指定" : item.id === "web-640" ? "横640px・高さ自動" : `${item.width} × ${item.height}`}</small>
          </button>`).join("")}
      </div>
    </div>`).join("");

  document.querySelectorAll("[data-preset]").forEach((button) => button.addEventListener("click", () => {
    const previousPresetId = state.presetId;
    state.presetId = button.dataset.preset;
    $("customSize").hidden = state.presetId !== "custom";
    applyPresetDefaults(previousPresetId);
    renderPresets();
    renderFits();
    renderFiles();
    updateSummary();
...
function renderFits() {
  if (isWeb640()) {
    $("fitList").innerHTML = `
      <div class="fit-option selected" aria-label="縦横比を維持">
        <span class="fit-icon contain"><i></i></span>
        <span><strong>縦横比を維持</strong><small>横幅640pxに合わせ、高さを自動計算します</small></span>
      </div>`;
    return;
  }
...
function updateSummary() {
  ...
  if (isWeb640()) {
    $("summarySize").textContent = "横640px・高さ自動";
    $("summaryDetails").textContent = `縦横比を維持 ・ ${format.toUpperCase()} ・ 画質${$("quality").value}%`;
    $("ratioPreview").style.aspectRatio = "4 / 3";
    return;
  }
...
function outputDimensionsText(item) {
  if (!isWeb640()) return "";
...
function renderFiles() {
  ...
  $("fileList").innerHTML = state.items.map((item) => `
    <article class="file-item" data-id="${item.id}">
      ...
      <div><input value="${item.outputName.replace(/&/g, "&amp;").replace(/"/g, "&quot;")}" aria-label="出力ファイル名"><small>${item.width} × ${item.height}px${outputDimensionsText(item)} ・ ${formatBytes(item.file.size)}</small></div>
...
async function downloadAll() {
  ...
  try {
    ...
    const { width, height, preset } = dimensions();
    const sizeLabel = isWeb640() ? `${width}w` : `${width}x${height}`;
    triggerDownload(await zip.generateAsync({ type: "blob" }), `resized_${preset.id}_${sizeLabel}.zip`);
